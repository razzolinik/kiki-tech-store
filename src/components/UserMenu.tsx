  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-4">
      ...
      <div className="relative bg-card rounded-3xl shadow-hover p-8 w-full max-w-sm text-center animate-scale-in">
        ...
        <h2 className="font-display font-bold text-xl text-foreground mb-1">Bienvenida a Kiki</h2>
        <p className="text-muted-foreground text-sm mb-6">Iniciá sesión para guardar tus favoritos y acceder a tu historial de compras.</p>
        <button onClick={() => googleLogin()} className="w-full flex items-center justify-center gap-3 bg-white border border-border rounded-xl px-4 py-3 text-sm font-medium text-foreground hover:bg-muted transition-colors shadow-soft">
        ...
        <p className="text-xs text-muted-foreground mt-4">Al continuar, aceptás nuestros términos y política de privacidad.</p>
      </div>
    </div>
  );
};

interface UserMenuProps { onOpenLogin: () => void; }
    ...
    return (
      <div className="relative" ref={menuRef}>
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2 rounded-full hover:bg-muted transition-colors px-2 py-1">
          <img src={user.picture} alt={displayName} className="h-8 w-8 rounded-full object-cover border-2 border-primary/30" referrerPolicy="no-referrer" crossOrigin="anonymous" />
            ...
            <div className="px-4 py-3 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-sm text-foreground line-clamp-1">{displayName}</p>
